...
static void set_factor(volume_context_t *vc, gfloat factor)
{
  vc->factor = factor;
#if 0
  if(vc->factor < 0.01f) vc->factor = 0.01f;
  if(vc->factor > 4.0f)  vc->factor = 4.0f;
#endif
#ifdef VERBOSE
...
void volume_init(volume_context_t *vc)
{
  memset(vc, 0, sizeof(*vc));
  vc->active = FALSE;
}

void volume_set_active(volume_context_t *vc, gboolean active)
{
  vc->active = active;
#ifdef VERBOSE
...
void volume_set_target_rms(volume_context_t *vc, gint target_rms)
{
...
void volume_set_song_rms(volume_context_t *vc, gint song_rms)
{
...
void volume_flow(volume_context_t *vc, gpointer *buffer, gint length)
{
  gint16 *in = *buffer;
  struct timeval tv;
  glong dt;
...
void volume_free(volume_context_t *vc)
{
}
