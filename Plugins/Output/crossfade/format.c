...
setup_format(AFormat fmt, gint rate, gint nch, format_t *format)
{
  ...
  /* calculate bps */
  format->bps = rate * nch;
  if(!format->is_8bit) format->bps *= 2;
...
{
  if(fmt1 == fmt2) return TRUE;  /* exact match */
...
#ifdef WORDS_BIGENDIAN
  if(((fmt1 == FMT_U16_NE) && (fmt2 == FMT_U16_BE))
  || ((fmt1 == FMT_S16_NE) && (fmt2 == FMT_S16_BE))) return TRUE;
#else
  if(((fmt1 == FMT_U16_NE) && (fmt2 == FMT_U16_LE))
  || ((fmt1 == FMT_S16_NE) && (fmt2 == FMT_S16_LE))) return TRUE;
#endif
...
{
  switch(fmt) {
  case FMT_U8:     return "FMT_U8";
  case FMT_S8:     return "FMT_S8";
  case FMT_U16_LE: return "FMT_U16_LE";
  case FMT_U16_BE: return "FMT_U16_BE";
  case FMT_U16_NE: return "FMT_U16_NE";
  case FMT_S16_LE: return "FMT_S16_LE";
  case FMT_S16_BE: return "FMT_S16_BE";
  case FMT_S16_NE: return "FMT_S16_NE";
  }
  return "UNKNOWN";
}
