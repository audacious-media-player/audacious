...
void
effect_init(effect_context_t *ec,
	    EffectPlugin     *ep)
{
...
void
effect_set_plugin(effect_context_t *ec,
		  EffectPlugin     *ep)
{
  if(ec->use_xmms_plugin && (ep == EFFECT_USE_XMMS_PLUGIN)) return;
  if(ec->ep == ep) return;
...
gint
effect_flow(effect_context_t *ec,
	    gpointer         *buffer,
	    gint              length,
	    format_t         *format,
  ...
  if(ec->use_xmms_plugin ? effects_enabled() : (ec->ep != NULL)) {
    ...
    if(ep != ec->last_ep) {
      DEBUG(("[crossfade] effect: plugin: \"%s\"%s\n",
	     ep ? plugin_name(ep) : "<none>",
	     ec->use_xmms_plugin ? " (XMMS)" : ""));
      ec->last_ep   = ep;
      ec->is_active = FALSE;
    }
    if(ep == NULL) return length;
	...
	if(allow_format_change) format_copy(format, &ec->format);
      }
    }
    else {
      ec->is_active = TRUE;
