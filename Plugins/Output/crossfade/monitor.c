...
xfade_update_monitor(gpointer userdata)
{
	...
	if (1 || monitor_win)
	{
	  ...
	  /*** Position ***/
		...
		{
			if (!bmp_playback_get_playing() || (monitor_closing == CLOSED))
			{	/* XMMS */
			...
			else
			{
				...
				g_snprintf(buffer, sizeof(buffer),
					   ...
					   ABS((position / 60000)), ABS((position / 1000) % 60), ABS((position / 100) % 10));
			...
			else
			{
				...
				if (diff)
				{
					...
					g_snprintf(buffer, sizeof(buffer),
						   ...
						   ABS((diff / 60000)), ABS((diff / 1000) % 60), ABS((diff) % 1000));
				...
				g_snprintf(buffer, sizeof(buffer),
					   ...
					   ABS((written_time / 1000) % 60), ABS((written_time / 100) % 10));
