...
static void update_plugin_config(gchar ** config_string, gchar * name, plugin_config_t * pc, gboolean save);
...
xfade_save_config()
{
	...
	bmp_cfg_db_set_string(db, section, "op_config_string",
			      config->op_config_string ? config->op_config_string : DEFAULT_OP_CONFIG_STRING);
...
xfade_cfg_fadein_len(fade_config_t * fc)
{
	...
	switch (fc->type)
	{
		...
		case FADE_TYPE_SIMPLE_XF:
			return fc->simple_len_ms;
		case FADE_TYPE_ADVANCED_XF:
			return fc->in_locked ? (fc->out_enable ? fc->out_len_ms : 0) : (fc->in_enable ? fc->in_len_ms : 0);
...
{
	return cfg->gap_trail_locked ? cfg->gap_lead_enable : cfg->gap_trail_enable;
}
...
xfade_cfg_gap_trail_len(config_t * cfg)
{
	if (!xfade_cfg_gap_trail_enable(cfg))
		return 0;
	return cfg->gap_trail_locked ? cfg->gap_lead_len_ms : cfg->gap_trail_len_ms;
}

gint
xfade_cfg_gap_trail_level(config_t * cfg)
{
	return cfg->gap_trail_locked ? cfg->gap_lead_level : cfg->gap_trail_level;
}
...
xfade_mix_size_ms(config_t * cfg)
{
	if (cfg->mix_size_auto)
	{
		...
		return min_size += xfade_cfg_gap_trail_len(cfg) + cfg->songchange_timeout;
	}
	else
...
update_plugin_config(gchar ** config_string, gchar * name, plugin_config_t * pc, gboolean save)
{
	...
	if (save)
	{
		...
		if ((pc->throttle_enable != default_pc.throttle_enable)
		    ...
		    || (pc->max_write_len != default_pc.max_write_len) || (pc->force_reopen != default_pc.force_reopen))
		{
			if (*out)
				strcat(out, "; ");
...
scan_plugins(GtkWidget * option_menu, gchar * selected)
{
	...
	while (list)
	{
		...
		/* create menu item */
		gtk_signal_connect(GTK_OBJECT(item), "activate", GTK_SIGNAL_FUNC(config_plugin_cb), (gpointer) index++);
		gtk_widget_show(item);
		gtk_menu_append(GTK_MENU(menu), item);
...
config_plugin_cb(GtkWidget * widget, gint index)
{
	...
	if (cfg->op_name)
		g_free(cfg->op_name);
	cfg->op_name = (op && op->filename) ? g_strdup(g_basename(op->filename)) : NULL;
...
scan_effect_plugins(GtkWidget * option_menu, gchar * selected)
{
	...
	while (list)
	{
		...
		/* create menu item */
		gtk_signal_connect(GTK_OBJECT(item), "activate", GTK_SIGNAL_FUNC(config_effect_plugin_cb), (gpointer) index++);
		gtk_widget_show(item);
		gtk_menu_append(GTK_MENU(menu), item);
...
config_effect_plugin_cb(GtkWidget * widget, gint index)
{
	...
	if (cfg->ep_name)
		g_free(cfg->ep_name);
	cfg->ep_name = (ep && ep->filename) ? g_strdup(g_basename(ep->filename)) : NULL;
...
create_crossfader_type_menu()
{
	...
	if ((optionmenu = lookup_widget(config_win, "xf_type_optionmenu")))
	{
		...
		if (mask & (1 << FADE_TYPE_REOPEN))
			add_menu_item(menu, "Reopen output device", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_REOPEN, &imap);
		if (mask & (1 << FADE_TYPE_FLUSH))
			add_menu_item(menu, "Flush output device", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_FLUSH, &imap);
		if (mask & (1 << FADE_TYPE_NONE))
			add_menu_item(menu, "None (gapless/off)", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_NONE, &imap);
		if (mask & (1 << FADE_TYPE_PAUSE))
			add_menu_item(menu, "Pause", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_PAUSE, &imap);
		if (mask & (1 << FADE_TYPE_SIMPLE_XF))
			add_menu_item(menu, "Simple crossfade", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_SIMPLE_XF, &imap);
		if (mask & (1 << FADE_TYPE_ADVANCED_XF))
			add_menu_item(menu, "Advanced crossfade", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_ADVANCED_XF, &imap);
		if (mask & (1 << FADE_TYPE_FADEIN))
			add_menu_item(menu, "Fadein", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_FADEIN, &imap);
		if (mask & (1 << FADE_TYPE_FADEOUT))
			add_menu_item(menu, "Fadeout", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_FADEOUT, &imap);
		if (mask & (1 << FADE_TYPE_PAUSE_NONE))
			add_menu_item(menu, "None", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_PAUSE_NONE, &imap);
		if (mask & (1 << FADE_TYPE_PAUSE_ADV))
			add_menu_item(menu, "Fadeout/Fadein", GTK_SIGNAL_FUNC(xf_type_cb), FADE_TYPE_PAUSE_ADV, &imap);
		gtk_option_menu_set_menu(GTK_OPTION_MENU(optionmenu), menu);
	}
}
...
on_xftfp_enable_check_toggled(GtkToggleButton * togglebutton, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].flush_pause_enable = gtk_toggle_button_get_active(togglebutton);
...
on_xftfp_length_spin_changed(GtkEditable * editable, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].flush_pause_len_ms = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
...
on_xftffi_enable_check_toggled(GtkToggleButton * togglebutton, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].flush_in_enable = gtk_toggle_button_get_active(togglebutton);
...
on_xftffi_length_spin_changed(GtkEditable * editable, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].flush_in_len_ms = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
	check_crossfader_dependencies(XF_FLUSH);
}
...
on_xftffi_volume_spin_changed(GtkEditable * editable, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].flush_in_volume = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
	check_crossfader_dependencies(XF_FLUSH);
}
...
on_pause_length_spin_changed(GtkEditable * editable, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].pause_len_ms = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
	check_crossfader_dependencies(XF_MIX_SIZE);
}
...
on_simple_length_spin_changed(GtkEditable * editable, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].simple_len_ms = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
	check_crossfader_dependencies(XF_MIX_SIZE);
}
...
on_fadeout_enable_check_toggled(GtkToggleButton * togglebutton, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].out_enable = gtk_toggle_button_get_active(togglebutton);
...
on_fadeout_length_spin_changed(GtkEditable * editable, gpointer user_data)
{
	if (checking)
		return;
	cfg->fc[cfg->xf_index].out_len_ms = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
...
{
	cfg->fc[cfg->xf_index].out_volume = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(editable));
...
{
	if (checking || !gtk_toggle_button_get_active(togglebutton))
		return;
...
void
on_presets_list_click_column(GtkCList * clist, gint column, gpointer user_data)
{
	DEBUG(("*** column=%d\n", column));
}
...
on_config_apply_clicked(GtkButton * button, gpointer user_data)
{
	...
	if (config->op_config_string)
		g_free(config->op_config_string);
	if (config->op_name)
		g_free(config->op_name);
	if (config->ep_name)
		g_free(config->ep_name);
...
xfade_configure()
{
	...
	if (!config_win)
	{
		...
		if (cfg->op_config_string)
			g_free(cfg->op_config_string);
		if (cfg->op_name)
			g_free(cfg->op_name);
		if (cfg->ep_name)
			g_free(cfg->ep_name);
...
xfade_about()
{
	...
	dialog = xmms_show_message(_("About crossfade"),
